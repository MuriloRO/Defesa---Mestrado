...
def update(strategy, detector, biometric_reference, new_features):
    ...
    elif strategy == 'GraphMinCutGrowing':
        new_model = GraphMinCutGrowing(detector, biometric_reference, new_features)
    elif strategy == 'GraphMinCutSliding':
        new_model = GraphMinCutSliding(detector, biometric_reference, new_features)
    else:
        print(strategy)
...
#funcionando como o growing
def GraphMinCutGrowing(detector, biometric_reference, new_features):
    biometric_reference.features = pandas.concat([biometric_reference.features, pandas.DataFrame(new_features, columns=biometric_reference.features.columns)], axis=0, ignore_index=True)
    new_model = detector.train(training_data=biometric_reference.features)
    return new_model

#funciona como o sliding
def GraphMinCutSliding(detector, biometric_reference, new_features): 
    biometric_reference.features = pandas.concat([biometric_reference.features, pandas.DataFrame(new_features, columns=biometric_reference.features.columns)], axis=0, ignore_index=True)
    biometric_reference.features = biometric_reference.features.iloc[len(new_features):]
    new_model = detector.train(training_data=biometric_reference.features)
    return new_model
